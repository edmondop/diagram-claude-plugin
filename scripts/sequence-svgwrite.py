...
def main() -> None:
    ...
    for i, name in enumerate(ACTORS):
        x = actor_x(i)
        box_w = 120
        dwg.add(dwg.rect(insert=(x - box_w / 2, HEADER_Y - 15),
                         size=(box_w, 30), rx=4, ry=4,
                         fill="#E3F2FD", stroke="#1976D2", stroke_width=1.5))
        dwg.add(dwg.text(name, insert=(x, HEADER_Y + 4),
                         text_anchor="middle", font_size="11px",
                         font_weight="bold", font_family=FONT, fill="#333"))
        # Lifeline
        line_end = FIRST_MSG_Y + len(MESSAGES) * MSG_SPACING + 20
        dwg.add(dwg.line(start=(x, HEADER_Y + 15), end=(x, line_end),
                         stroke="#BDBDBD", stroke_width=1, stroke_dasharray="4,4"))

    # Arrowhead marker
    marker = dwg.marker(id="arrow", insert=(6, 3), size=(6, 6),
    ...
    for i, msg in enumerate(MESSAGES):
        ...
        line = dwg.line(start=(x1, y), end=(x2, y),
        ...
        label_x = (x1 + x2) / 2
        dwg.add(dwg.text(msg.label, insert=(label_x, y - 8),
                         text_anchor="middle", font_size="10px",
                         font_family=FONT, fill="#333"))
